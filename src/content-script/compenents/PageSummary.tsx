...
function PageSummary(props: Props) {
    ...
    const onSummary = useCallback(async () => {
        ...
        const description =
            article?.description || document.querySelector('meta[name="description"]')?.getAttribute('content') || ''
        ...
        if (article?.content || description) {
            ...
            const prompt = pageComments?.content
                ? commentSummaryPrompt({
                      ...
                      prompt: userConfig.promptComment ? userConfig.promptComment : pageSummaryPromptHighlight,
                      rate: article?.['rate'],
                  })
                : pageSummaryPrompt({
                      content: promptContent,
    ...
    useEffect(() => {
        ...
        const show = pageSummaryEnable && ((isIOS && inWhitelist) || (inWhitelist && !siteRegex?.test(hostname)))
                ...
                <div className="glarity--card">
                    <div className="glarity--card__head ">
                        ...
                        <div className="glarity--card__head--action">
                            <button className={classNames('glarity--btn', 'glarity--btn__icon')} onClick={onSwitch}>
                                <XCircleFillIcon />
                            </button>
                        </div>
                    </div>
