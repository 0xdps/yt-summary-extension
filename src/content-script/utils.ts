...
export function getPossibleElementByQuerySelector<T extends Element>(queryArray: string[]): T | undefined {
...
export async function getLangOptionsWithLink(videoId) {
    ...
    return Array.from(languageOptions).map((langName, _index) => {
...
export async function getTranscriptHTML(rawTranscript: string[], _videoId) {
    const scriptObjArr = [],
        timeUpperLimit = 60,
        charInitLimit = 300,
        charUpperLimit = 500
...
export function matchSites(site: string) {
    return /(^(www\.)?youtube\.com)/.test(site)
}

export const hostname = location.hostname

export function siteName() {
    return hostname.includes('youtube.com') ? 'youtube' : ''
}
